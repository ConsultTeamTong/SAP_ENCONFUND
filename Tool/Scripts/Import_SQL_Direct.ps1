...
param(
    ...
    [string]$MapFile    = "",
    [string]$RptRoot    = "$PSScriptRoot\..\ExportedFromSAP",
...
# Default map = the per-DB export index (RptRoot\_ExportIndex.csv)
if ([string]::IsNullOrWhiteSpace($MapFile)) { $MapFile = Join-Path $RptRoot "_ExportIndex.csv" }

Write-Log "=== Start SQL Direct Import ==="
Write-Log "MapFile: $MapFile"
Write-Log "RptRoot: $RptRoot"
Write-Log "DryRun : $DryRun"

if (-not (Test-Path $MapFile)) { Write-Log "MapFile not found: $MapFile" "ERROR"; exit 1 }

if ($MapFile -match '\.csv$') {
    # _ExportIndex.csv: files sit DIRECTLY under RptRoot (per-DB folder),
    # so RPT_Folder is forced empty -> rptPath = RptRoot\RPT_FileName.
    $mapRows = Import-Csv -Path $MapFile -Encoding UTF8 | ForEach-Object {
        [PSCustomObject]@{
            No           = $_.No
            Module       = $_.Module
            RPT_FileName = $_.RPT_FileName
            RPT_Folder   = ""
...
foreach ($row in $mapRows) {
    ...
    if ([string]::IsNullOrWhiteSpace($row.RPT_Folder)) {
        $rptPath = Join-Path $RptRoot $row.RPT_FileName              # flat: files directly under RptRoot
    } else {
