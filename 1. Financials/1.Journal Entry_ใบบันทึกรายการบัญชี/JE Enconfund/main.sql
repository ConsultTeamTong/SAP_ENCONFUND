-- ============================================================
-- Report: JE Enconfund.rpt
Path:   1. Financials\1.Journal Entry_ใบบันทึกรายการบัญชี\JE Enconfund.rpt
Extracted: 2026-07-01 09:27:59
-- Source: Main Report
-- Table:  main
-- ============================================================
...
SELECT
    ...
    RC."ReconNum",
    COALESCE(
    IFNULL(AP3."BeginStr", '') || CAST(AP1."DocNum" AS NVARCHAR({?DocKey@})),
    IFNULL(AR3."BeginStr", '') ||  CAST(AR1."DocNum" AS NVARCHAR({?DocKey@})),
    IFNULL(NC3."BeginStr", '') ||  CAST(NC1."DocNum" AS NVARCHAR({?DocKey@})),
    IFNULL(OT3."BeginStr", '') || CAST(OT1."DocNum" AS NVARCHAR({?DocKey@})),
    IFNULL(NC3."BeginStr", '') ||  CAST(NC1."DocNum" AS NVARCHAR({?DocKey@}))
	--IFNULL(T6."BeginStr", '') ||  CAST(HEAD."Number" AS NVARCHAR({?DocKey@}))
    ) AS "เลขที่เอกสาร",
 ...
 TO_VARCHAR(ADD_YEARS( HEAD."CreateDate", 543), 'DD.MM.YYYY') AS "DateThai",
    IFNULL(T6."BeginStr", '') ||  CAST(HEAD."Number" AS NVARCHAR({?DocKey@})) AS "เลขที่เอกสาร JE",
...
LEFT JOIN  "{?Schema@}".OVPM OT2 ON OT1."CardCode" = OT2."CardCode"
