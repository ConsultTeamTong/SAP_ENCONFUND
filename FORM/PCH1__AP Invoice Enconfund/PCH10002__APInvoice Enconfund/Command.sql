...
LEFT JOIN {?Schema@}.ODRF odrf ON T0."draftKey" = odrf."DocEntry"
LEFT JOIN (
    SELECT 
        T_Line."DocEntry",
...
LEFT JOIN (
    SELECT 
        ...
        MAX(CASE WHEN w."SortId" = '1' AND w."Status" = 'Y' THEN w."UpdateDate" END) AS "เวลาผู้ตรวจ",
        ...
        MAX(CASE WHEN w."SortId" = '2' AND w."Status" = 'Y' THEN w."UpdateDate" END) AS "เวลาผู้อนุมัติ"
    FROM {?Schema@}."OWDD" o 
