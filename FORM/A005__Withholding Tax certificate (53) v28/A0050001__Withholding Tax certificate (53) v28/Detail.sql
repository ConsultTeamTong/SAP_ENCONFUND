...
Path:   FORM\A005__Withholding Tax certificate (53) v28\A0050001__Withholding Tax certificate (53) v28.rpt
Extracted: 2026-07-01 09:28:01
-- Source: Main Report
-- Table:  Detail
-- ============================================================
		...
		FROM (
				...
				WHERE "OVPM"."TransId" IN (SELECT "TransId" FROM {?Schema@}."JDT1" WHERE "JDT1"."ShortName"
											= (SELECT "AcctCode" FROM {?Schema@}."OACT" WHERE "FormatCode"
											 	= (SELECT "Name" FROM {?Schema@}."@SLDT_SET_ACCWH" WHERE "U_SLD_TypeWH" = '53'
											 		)
											 	)
											)
				----------------------------------------------------------------------------------------------------------------------
				...
				AND "JDT2"."Account" = (SELECT "AcctCode" FROM {?Schema@}."OACT" WHERE "FormatCode" = (SELECT "Name" FROM {?Schema@}."@SLDT_SET_ACCWH" WHERE "U_SLD_TypeWH" = '53'))
				WHERE "JDT1"."Credit" > 0 				
				----------------------------------------------------------------------------------------------------------------------
				UNION ALL 
				----------------------------------------------------------------------------------------------------------------------
				SELECT DISTINCT	'"OJDT"' AS "TYPE",
						...
						"JDT1"."U_SLD_LPBranch" AS "U_SLD_VBPBranch",
						"JDT1"."U_SLD_WhPayBy",
						(SELECT "OWHT"."WTCode" FROM  {?Schema@}."OWHT" WHERE "OWHT"."WTCode" = "JDT1"."U_SLD_WTCode" AND "JDT1"."ShortName" = (SELECT "AcctCode" FROM  {?Schema@}."OACT" WHERE "FormatCode" = (SELECT "Name" FROM  {?Schema@}."@SLDT_SET_ACCWH" WHERE "U_SLD_TypeWH" = '53'))) "WTCode"
						
				FROM {?Schema@}."JDT1" 
				LEFT JOIN {?Schema@}."OJDT"  ON "JDT1"."TransId" = "OJDT"."TransId"
				WHERE "JDT1"."ShortName" = (SELECT "AcctCode" FROM {?Schema@}."OACT" WHERE "FormatCode" = (SELECT "Name" FROM {?Schema@}."@SLDT_SET_ACCWH" WHERE "U_SLD_TypeWH" = '53'))                                           
