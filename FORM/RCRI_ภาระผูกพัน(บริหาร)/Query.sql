WITH POLines AS (
    ...
    FROM {?schema@}.OAT1 A
    INNER JOIN {?schema@}.POR1 P ON A."AgrNo" = P."AgrNo" AND A."AgrLineNum" = P."AgrLnNum"
),
AgrTotals AS (
    SELECT "AgrNo", SUM("UnitPrice") AS "CalculatedOpenAmount", SUM(CASE WHEN YEAR("ShipDate") - CAST('{?Year@}' AS INT) <= 5 THEN "UnitPrice" ELSE 0 END) AS "Amt_1_5", SUM(CASE WHEN YEAR("ShipDate") - CAST('{?Year@}' AS INT) > 5 THEN "UnitPrice" ELSE 0 END) AS "Amt_Over_5"
    FROM POLines
    GROUP BY "AgrNo"
),
InvoicedLines AS (
    ...
    INNER JOIN {?schema@}.PDN1 ON OOAT."AbsID" = PDN1."AgrNo"
    INNER JOIN {?schema@}.PCH1 ON PDN1."DocEntry" = PCH1."BaseEntry" AND PDN1."LineNum" = PCH1."BaseLine" AND PCH1."BaseType" = 20
    INNER JOIN {?schema@}.OPCH ON PCH1."DocEntry" = OPCH."DocEntry"
)
SELECT DISTINCT
    INV."AbsID",
    INV."Number",
...
LEFT JOIN (
    SELECT V2."DocEntry" AS "InvEntry", SUM(CASE WHEN P."Category" < '{?Year@}' THEN V2."SumApplied" ELSE 0 END) AS "OB_Amount", 
    SUM(CASE WHEN P."Category" = '{?Year@}' THEN V2."SumApplied" ELSE 0 END) AS "Movement_Amount", 
    SUM(CASE WHEN P."Category" <= '{?Year@}' THEN V2."SumApplied" ELSE 0 END) AS "PaidAmount", 
    COUNT(DISTINCT CASE WHEN P."Category" <= '{?Year@}' THEN V0."DocEntry" END) AS "PayCount"
    FROM {?schema@}.VPM2 V2 
    INNER JOIN {?schema@}.OVPM V0 ON V2."DocNum" = V0."DocEntry"
    INNER JOIN {?schema@}.OFPR P ON V0."DocDate" >= P."F_RefDate" AND V0."DocDate" <= P."T_RefDate"
