...
Path:   FORM\INV1__AR INVOICE Enconfund (1)\INV10004__AR INVOICE Enconfund (1).rpt
Extracted: 2026-07-01 09:28:06
-- Source: Main Report
-- Table:  Command
-- ============================================================
...
SELECT 
    ...
    ODRF."CreateDate" AS "วันที่ทำรายการ",
...
LEFT JOIN (
      SELECT DISTINCT
          ...
          RC."ReconNum"
      FROM {?Schema@}."ITR1" RC
      INNER JOIN {?Schema@}."OITR" HC
          ON HC."ReconNum" = RC."ReconNum"
         AND HC."Canceled" = 'N'
  ) RC ON HEAD."TransId" = RC."TransId"
